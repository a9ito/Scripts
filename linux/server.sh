...
while true; do
    ...
    case $yn in
        [Yy]* )
            ...
            echo UUID=`sudo blkid -s UUID -o value /dev/sdb` /mnt/ ext4 discard,defaults,nofail 0 2 | sudo tee -a /etc/fstab;
            cat /etc/fstab; break;;
        [Nn]* ) break;;
...
cd$HOME
wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1eV2q-YpOSwMaFyoyeKqxjq-HJpgWRy7l' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1eV2q-YpOSwMaFyoyeKqxjq-HJpgWRy7l" -O gerrit.zip && rm -rf /tmp/cookies.txt
unzip gerrit.zip
sed -i 's/8080/2222/g' /home/gerrit/gerrit/etc/gerrit.config
